...
# # --- Load & Split Word Documents (.docx) ---
# def load_and_split_documents():
#     docs = []
#     for filename in os.listdir("data"):
#         if filename.endswith(".docx"):
#             loader = UnstructuredWordDocumentLoader(os.path.join("data", filename))
#             docs.extend(loader.load())
#     splitter = RecursiveCharacterTextSplitter(chunk_size=1000, chunk_overlap=100)
#     return splitter.split_documents(docs)
...
def main():
    ...
    # # Upload DOCX Files
    # uploaded_files = st.file_uploader("Upload Word (.docx) files:", type=["docx"], accept_multiple_files=True)
    # if uploaded_files:
    #     os.makedirs("data", exist_ok=True)
    #     for f in uploaded_files:
    #         file_path = os.path.join("data", f.name)
...
#     for filename in os.listdir("data"):
#         if filename.endswith(".docx"):
#             loader = Docx2txtLoader(os.path.join("data", filename))
#             docs.extend(loader.load())
#     splitter = RecursiveCharacterTextSplitter(chunk_size=1000, chunk_overlap=100)
#     return splitter.split_documents(docs)
...
# # --- Prompt Template ---
# prompt_template = """
# Use provided data to generate answers of relative query (max 250 words). 
# If no relevant answer is found, politely ask the user to elaborate further.
...
# # --- Load LLM from Ollama ---
# def get_ollama_llm():
#      return ChatOllama(model="deepseek-r1:1.5b")  # ✅ Matches the model you installed
...
# def main():
#     st.set_page_config(page_title="Chat with Word Docs")
#     st.header("📄 Chat with Word Documents (.docx) using Ollama 💬")
...
#                     st.success("✅ Vector store updated successfully!")
#                 else:
#                     st.warning("⚠️ No valid Word documents found to index.")
...
# ✅ Yes, your entire code (excluding the commented parts) looks structurally correct and clean.
# You’ve handled:

# ✅ File uploading with .docx support.

# ✅ Embedding setup with OllamaEmbeddings.

# ✅ Document splitting using RecursiveCharacterTextSplitter.

# ✅ Vector store creation with FAISS and saving it locally.

# ✅ Prompt template with context-based answering.

# ✅ Retrieval and LLM chain using RetrievalQA.

# ✅ Streamlit UI with proper checks:
